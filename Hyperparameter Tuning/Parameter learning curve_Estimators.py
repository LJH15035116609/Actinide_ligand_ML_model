...
# 存储所有折的R²分数和估计器数量
all_r2_scores = []
num_estimators = list(range(1, 501, 1))
...
df_avg_r2_scores.to_csv('avg_r2_scores.csv', index=False)

# 绘制R²分数与估计器数量的图表
plt.plot(num_estimators, avg_r2_scores)
plt.xlabel('Number of Estimators')
plt.xticks(range(1, 501, 50))
plt.ylabel('Average R2 Score')
plt.title('Gradient Boosting Average R2 Score vs. Number of Estimators')
plt.grid(True)
plt.savefig('GradientBoostingRegressor_r2_Number of Estimators.png', bbox_inches='tight')  # 保存图像并且调整边界
plt.show()
