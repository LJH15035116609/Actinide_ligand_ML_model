...
i = 0
for line in lines:
    if re.match(r'^Atom\s+q\(N\)', line.strip()):
        index = i
        indexs.append(index)
    i += 1
...
for index in indexs:
    while True:
        ...
        if index == len(lines) or re.match(r'^\s*$', lines[index]):
            ...
            extract_list.append(f'{j}    '+lines[index].strip())
    j += 1

# 将提取出的列表中的元素中间的空格替换为’,‘后存入csv文件，表头为‘Atom,q(N),q(N+1),q(N-1),f-,f+,f0,CDD’
with open('./extract_atom1.csv', 'w') as f:
    f.write('number,Atom,q(N),q(N+1),q(N-1),f-,f+,f0,CDD\n')
